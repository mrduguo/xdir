...
setupClasspath() {
    ...
    CLASSPATH=$XDIR_HOME/classes:$XDIR_HOME/lib/*
}
...
main() {
    ...
    FINAL_COMMAND="$JAVA  -classpath $CLASSPATH -Dfile.encoding=UTF-8 -Dxdir.home=$XDIR_HOME $XDIR_OPTS $@ org.duguo.xdir.infra.spring.server.Main $ACTION"

    #echo "FINAL_COMMAND: $FINAL_COMMAND"
    if [ "$ACTION" = "start" ]; then
        mkdir -p $XDIR_HOME/var/log
	    nohup $FINAL_COMMAND >& $XDIR_HOME/var/log/console.log </dev/null &
	    sleep 2
        echo "XDir Started with log at $XDIR_HOME/var/log/xdir.log"
    else
        $FINAL_COMMAND
    fi
    EXIT_CODE="$?"
