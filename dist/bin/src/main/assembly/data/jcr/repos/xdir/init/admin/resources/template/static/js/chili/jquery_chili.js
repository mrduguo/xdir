...
===============================================================================
+credit http://thisblog.usejquery.com/2009/03/11/highlight-your-source-code-with-jquery-and-chili/
*/
...
// options --------------------------------------------------------------------
	...
	, automatic:          true
	, automaticSelector:  "pre.cplusplus,pre.csharp,pre.css,pre.delphi,pre.html,pre.java,pre.js,pre.lotusscript,pre.mysql,pre.php"
	...
	, recipeLoading:      true
	, recipeFolder:       "${getResourceUrl("js/chili/")}" // used like: recipeFolder + recipeName + '.js'
...
// the coloring starts here
	...
	.each( function() {
		...
		$this.trigger( 'chili.after_coloring' );
	        $this.addClass("code")
		.before('<a href="#" class="codeswitch" style="float:right;margin:5px 5px 0 0;">View this source in Plain Text</a>')
      		.after('<textarea rows="' + ($this.text().split("\n").length) + '" style="display:none;width:99%;">' + $this.text() + '</textarea>');
	} );
...
$( function() {
	...
	if( ChiliBook.automatic ) {
		  ...
		  $('.codeswitch').toggle(function() {
		    $(this)
		      .text('View this source as Highlighted Code')
		      .next().hide().next().show();
		  }, function() {
		    $(this)
